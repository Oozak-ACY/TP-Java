...
public class Balle extends Sprite{
    ...
    public void collisionManager(Barre barre) {
        if (y + diametre >= barre.y && x >= barre.x && x <= barre.x + barre.largeur) {
            rebondY();
        }
    }

    public boolean briqueTouched(Brique brique) {
        Rectangle ballePosition = new Rectangle(x, y, diametre, diametre);
        Rectangle briquePosition = new Rectangle(brique.x, brique.y, brique.largeur, brique.hauteur);
        return ballePosition.intersects(briquePosition);
    }
